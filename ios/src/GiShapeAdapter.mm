...
void GiShapeAdapter::setPen(int argb, float width, int style, float phase, float)
{
    ...
    if (style >= 0) {
        ...
        if (linecap & kLineCapButt)
            ...
            _container.lineCapStyle = ((style > 0 && style < 5)
...
CALayer *GiShapeCallback::layer()
{
    return _rootLayer ? _rootLayer : _shapeLayer;
}
...
{
    if (!_rootLayer && _shapeLayer) {
        [_shapeLayer.sublayers makeObjectsPerformSelector:@selector(removeFromSuperlayer)];
    }
